...
async function loadComponentsForOrder(orderId, baseComponents, variants = {}, allItems = []) {
    ...
    const updateComponentIfExistsOrAllowedExtra = (componentType, newValue, context = '') => {
        ...
        if (normalizedType === 'SSD ADDON') {
            ...
            console.log(`ℹ️ Extra aggiunto (${context || 'OperatoreA'}): SSD ADDON`);
            return true;
        }

        console.warn(`⚠️ Variante ignorata (${context || 'OperatoreA'}): componente ${componentType} non presente nella configurazione standard`);
        return false;
    };
    ...
    for (const [key, value] of Object.entries(variants)) {
        ...
        if (splitResult.ram && splitResult.ssd) {
            ...
            updateComponentIfExistsOrAllowedExtra('RAM', ramValue, 'OperatoreA split RAM');
            ...
            updateComponentIfExistsOrAllowedExtra('SSD', ssdValue, 'OperatoreA split SSD');
        ...
        if (gpoMappableTypes.includes(gpoSearchType)) {
            ...
            if (gpoMatch) {
                ...
                if (componentIndex !== -1) {
                    ...
                    updateComponentIfExistsOrAllowedExtra(baseComponentType, variantValue, `OperatoreA GPO ${gpoSearchType}`);
                }
                continue; 
            }
        ...
        if (componentIndex !== -1) {
            ...
            updateComponentIfExistsOrAllowedExtra(baseComponentType, variantValue, `OperatoreA variant ${baseComponentType}`);
        }
    }
